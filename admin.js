...
function viewOrder(id) {
  ...
  showModal(`
    ...
    ${pm==='instapay'?`
    <div style="border:1px solid rgba(255,107,53,.25);border-radius:var(--radius);padding:16px;background:rgba(255,107,53,.04);margin-bottom:16px">
...
function renderAnalytics() {
  ...
  setTimeout(()=>{
    ...
    if(ctx&&typeof Chart!=='undefined'){if(Admin.charts.revenue)Admin.charts.revenue.destroy();Admin.charts.revenue=new Chart(ctx,{type:'line',data:{labels:Object.keys(dayMap),datasets:[{label:'Revenue',data:Object.values(dayMap),borderColor:'#FF6B35',backgroundColor:'rgba(255,107,53,.08)',borderWidth:2,fill:true,tension:.4,pointRadius:4,pointBackgroundColor:'#FF6B35'}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{color:'#666',font:{size:11}},grid:{color:'rgba(255,255,255,.04)'}},y:{ticks:{color:'#666',font:{size:11},callback:v=>`ج.م ${v}`},grid:{color:'rgba(255,255,255,.04)'}}}}})}
    const pmCtx=document.getElementById('pmChart');
    if(pmCtx&&typeof Chart!=='undefined'){if(Admin.charts.pm)Admin.charts.pm.destroy();Admin.charts.pm=new Chart(pmCtx,{type:'doughnut',data:{labels:['Cash','Card','InstaPay'],datasets:[{data:[pmCounts.cash,pmCounts.card,pmCounts.instapay],backgroundColor:['#22c55e','#3b82f6','#FF6B35'],borderWidth:0}]},options:{plugins:{legend:{position:'bottom',labels:{color:'#888',font:{size:12}}}},cutout:'65%',maintainAspectRatio:false}})}
  ...
  try {
    ...
    grid.innerHTML=tables.map(tbl=>{
      ...
      return `<div onclick="navigate('orders');filterOrders('all');searchOrders('${tbl.table_number}')"
        style="background:${color};border:2px solid ${active.length?'rgba(255,107,53,.3)':'var(--border)'};border-radius:16px;padding:18px 10px;text-align:center;cursor:pointer;transition:all .2s">
  ...
  document.getElementById('page').innerHTML=`
    <div class="settings-grid">
      ...
      <div class="settings-card" style="grid-column:1/-1">
        ...
        <p style="font-size:13px;color:var(--text2);margin-bottom:20px">Customize colors, fonts, and visual style of the customer-facing menu site.</p>

        <!-- Accent color -->
        <div style="margin-bottom:24px">
          <div style="font-size:12px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:1px;margin-bottom:12px">Accent Color</div>
          <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap">
            <input type="color" value="${s.accent_color||'#FF6B35'}" id="accentColorPicker" oninput="updateAccentLive(this.value)" style="width:52px;height:40px;border:none;cursor:pointer;border-radius:8px;background:transparent">
            <div style="display:flex;gap:8px;flex-wrap:wrap">
              ${['#FF6B35','#e11d48','#7c3aed','#2563eb','#0891b2','#059669','#d97706','#dc2626'].map(c=>`
                <button onclick="updateAccentLive('${c}');document.getElementById('accentColorPicker').value='${c}'"
                  style="width:32px;height:32px;border-radius:50%;background:${c};border:3px solid ${(s.accent_color||'#FF6B35')===c?'#fff':'transparent'};cursor:pointer;transition:border-color .2s" title="${c}"></button>`).join('')}
            </div>
            <button class="btn btn-primary btn-sm" onclick="saveAccentColor()">Apply</button>
          </div>
        </div>

        <!-- Dark/Light mode toggle for menu -->
        <div style="margin-bottom:24px">
          <div style="font-size:12px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:1px;margin-bottom:12px">Menu Site Theme</div>
          <div style="display:flex;gap:10px;flex-wrap:wrap">
            ${[['dark','🌙 Dark (default)','#0a0a0a'],['midnight','🔵 Midnight Blue','#070b14'],['forest','🌿 Forest Dark','#071210'],['wine','🍷 Deep Wine','#12070a'],['light','☀️ Light','#f8f8f6']].map(([key,label,bg])=>`
              <button onclick="applyThemePreset('${key}')" id="theme-btn-${key}"
                style="padding:10px 18px;border-radius:12px;font-size:13px;font-weight:600;cursor:pointer;border:2px solid ${(s.theme_preset||'dark')===key?'var(--accent)':'var(--border2)'};background:${bg};color:${key==='light'?'#111':'#f0f0f0'};transition:all .2s">
                ${label}
              </button>`).join('')}
          </div>
        </div>
        ...
        <!-- Hero text -->
        <div style="margin-bottom:24px">
          <div style="font-size:12px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:1px;margin-bottom:12px">Hero Section Text</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
            <div class="form-group">
              <label class="form-label">Hero Title Line 1</label>
              <input class="form-control" id="setting_hero_title1" value="${s.hero_title1||'Crafted with'}" placeholder="Crafted with">
            </div>
            <div class="form-group">
              <label class="form-label">Hero Title Line 2 (accent)</label>
              <input class="form-control" id="setting_hero_title2" value="${s.hero_title2||'Passion.'}" placeholder="Passion.">
            </div>
            <div class="form-group">
              <label class="form-label">Hero Subtitle</label>
              <input class="form-control" id="setting_hero_subtitle" value="${s.hero_subtitle||'An elevated dining experience where every dish tells a story.'}" placeholder="Subtitle text…">
            </div>
            <div class="form-group">
              <label class="form-label">Tagline (footer)</label>
              <input class="form-control" id="setting_tagline" value="${s.tagline||'Crafted with passion. Served with love.'}" placeholder="Footer tagline…">
            </div>
          </div>
          <button class="btn btn-primary btn-sm" onclick="saveHeroText()">Save Text</button>
        </div>

        <!-- Hero stats -->
        <div style="margin-bottom:24px">
          <div style="font-size:12px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:1px;margin-bottom:12px">Hero Stats (shown on homepage)</div>
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px">
            ${[['stat1_num','Stat 1 Number',s.stat1_num||'12+'],['stat1_label','Stat 1 Label',s.stat1_label||'Signature Dishes'],
               ['stat2_num','Stat 2 Number',s.stat2_num||'4.9★'],['stat2_label','Stat 2 Label',s.stat2_label||'Guest Rating'],
               ['stat3_num','Stat 3 Number',s.stat3_num||'20 min'],['stat3_label','Stat 3 Label',s.stat3_label||'Avg Prep Time']
              ].map(([key,label,val])=>`<div class="form-group"><label class="form-label">${label}</label><input class="form-control" id="setting_${key}" value="${val}" placeholder="${label}"></div>`).join('')}
          </div>
          <button class="btn btn-primary btn-sm" onclick="saveHeroStats()">Save Stats</button>
        </div>

        <!-- Menu section labels -->
        <div style="margin-bottom:24px">
          <div style="font-size:12px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:1px;margin-bottom:12px">Menu Section Labels</div>
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px">
            ${[['menu_section_label','Section Label (EN)',s.menu_section_label||'Full Menu'],
               ['menu_section_title','Section Title (EN)',s.menu_section_title||'What are you craving today?'],
               ['menu_section_label_ar','Section Label (AR)',s.menu_section_label_ar||'القائمة الكاملة'],
              ].map(([key,label,val])=>`<div class="form-group"><label class="form-label">${label}</label><input class="form-control" id="setting_${key}" value="${val}"></div>`).join('')}
          </div>
          <button class="btn btn-primary btn-sm" onclick="saveMenuLabels()">Save Labels</button>
        </div>

        <!-- Border radius / card style -->
        <div style="margin-bottom:24px">
          <div style="font-size:12px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:1px;margin-bottom:12px">Card Corner Style</div>
          <div style="display:flex;gap:10px;flex-wrap:wrap">
            ${[['sharp','▪ Sharp','0px'],['soft','◼ Soft','12px'],['rounded','⬛ Rounded (default)','20px'],['pill','⬤ Very Rounded','28px']].map(([key,label,val])=>`
              <button onclick="applyBorderRadius('${val}','${key}')" id="radius-btn-${key}"
                style="padding:10px 18px;border-radius:${val};border:2px solid ${(s.card_radius||'20px')===val?'var(--accent)':'var(--border2)'};background:var(--dark3);color:var(--text);font-size:13px;cursor:pointer;transition:all .2s">
                ${label}
              </button>`).join('')}
          </div>
        </div>
        ...
        <div style="margin-bottom:8px">
          <div style="font-size:12px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:1px;margin-bottom:12px">Hero Font Style</div>
          <div style="display:flex;gap:10px;flex-wrap:wrap">
            ${[['playfair','Playfair Display (default)','Playfair Display'],['inter','Inter (modern)','Inter'],['serif','Georgia (classic)','Georgia']].map(([key,label,font])=>`
              <button onclick="applyHeroFont('${font}','${key}')" id="font-btn-${key}"
                style="padding:10px 18px;border-radius:12px;border:2px solid ${(s.hero_font||'Playfair Display')===font?'var(--accent)':'var(--border2)'};background:var(--dark3);color:var(--text);font-size:14px;font-family:'${font}',serif;cursor:pointer;transition:all .2s">
                ${label}
              </button>`).join('')}
          </div>
        </div>
      ...
      return `<div class="table-card" style="border-radius:var(--radius2)">
        <div style="padding:20px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between">
          <div style="display:flex;align-items:center;gap:12px">
            <div style="width:46px;height:46px;border-radius:12px;background:rgba(255,107,53,.1);border:1px solid rgba(255,107,53,.2);display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:800;color:var(--accent)">${tbl.table_number}</div>
...
function printSelectedReceipts(){const ids=getSelectedOrderIds();if(!ids.length){toast('Select at least one order','error');return;}openPrintWindow(ids.map(id=>Admin.orders.find(o=>o.id===id)).filter(Boolean));}
function openPrintWindow(orders){const s=Admin.settings;const name=s.restaurant_name||'Flavor House';const logo=s.restaurant_logo||'';const currency=s.currency_symbol||'ج.م';const fmtP=n=>`${currency} ${Math.round(n).toLocaleString()}`;const pmLabels={cash:'Cash on Arrival',card:'Card on Arrival',instapay:'InstaPay Egypt'};const receiptsHtml=orders.map((o,idx)=>{const pm=pmLabels[o.payment_method||'cash']||o.payment_method||'Cash';const date=new Date(o.created_at?.toDate?o.created_at.toDate():o.created_at||Date.now()).toLocaleString('en-GB',{day:'2-digit',month:'short',year:'numeric',hour:'2-digit',minute:'2-digit'});return`<div class="receipt${idx<orders.length-1?' page-break':''}"><div class="receipt-header">${logo?`<img src="${logo}" class="receipt-logo">`:`<div class="receipt-logo-placeholder">${name.charAt(0)}</div>`}<div class="receipt-name">${name}</div>${s.tagline?`<div class="receipt-tagline">${s.tagline}</div>`:''}</div><div class="receipt-divider">- - - - - - - - - - - - - - -</div><div class="receipt-meta"><div class="receipt-meta-row"><span>Order</span><span><strong>#${o.order_number}</strong></span></div><div class="receipt-meta-row"><span>Table</span><span>${o.table_number}</span></div>${o.customer_name?`<div class="receipt-meta-row"><span>Customer</span><span>${o.customer_name}</span></div>`:''}<div class="receipt-meta-row"><span>Date</span><span>${date}</span></div></div><div class="receipt-divider">- - - - - - - - - - - - - - -</div><table class="receipt-items"><thead><tr><th style="text-align:left">Item</th><th style="text-align:center">Qty</th><th style="text-align:right">Price</th><th style="text-align:right">Total</th></tr></thead><tbody>${o.items?.map(i=>`<tr><td>${i.name}${i.name_ar?`<br><span style="font-size:10px;opacity:.7;direction:rtl">${i.name_ar}</span>`:''}</td><td style="text-align:center">${i.qty}</td><td style="text-align:right">${fmtP(i.price)}</td><td style="text-align:right">${fmtP(i.price*i.qty)}</td></tr>`).join('')||''}</tbody></table><div class="receipt-divider">- - - - - - - - - - - - - - -</div><div class="receipt-totals">${o.subtotal!=null?`<div class="receipt-total-row"><span>Subtotal</span><span>${fmtP(o.subtotal)}</span></div>`:''} ${o.tax&&o.tax>0?`<div class="receipt-total-row"><span>VAT</span><span>${fmtP(o.tax)}</span></div>`:''} ${o.service_charge&&o.service_charge>0?`<div class="receipt-total-row"><span>Service</span><span>${fmtP(o.service_charge)}</span></div>`:''} ${o.discount&&o.discount>0?`<div class="receipt-total-row" style="color:#16a34a"><span>Discount</span><span>-${fmtP(o.discount)}</span></div>`:''}<div class="receipt-total-row grand"><span>TOTAL</span><span>${fmtP(o.total)}</span></div></div><div class="receipt-divider">- - - - - - - - - - - - - - -</div><div class="receipt-payment">Payment: <strong>${pm}</strong></div>${o.notes?`<div class="receipt-notes">Note: ${o.notes}</div>`:''}<div class="receipt-footer"><div>Thank you for dining with us!</div>${s.wifi_name?`<div style="margin-top:4px;font-size:10px">WiFi: ${s.wifi_name}${s.wifi_pass?` · Pass: ${s.wifi_pass}`:''}</div>`:''}</div></div>`;}).join('');const win=window.open('','_blank','width=420,height=700');win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Receipts</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Courier New',monospace;font-size:12px;background:#fff;color:#111}.receipt{width:72mm;margin:0 auto;padding:12px 8px 20px}.page-break{page-break-after:always;border-bottom:3px dashed #ccc;margin-bottom:24px;padding-bottom:24px}.receipt-header{text-align:center;margin-bottom:10px}.receipt-logo{max-width:80px;max-height:60px;object-fit:contain;margin:0 auto 6px;display:block}.receipt-logo-placeholder{width:52px;height:52px;border-radius:50%;background:#111;color:#fff;font-size:24px;font-weight:800;display:flex;align-items:center;justify-content:center;margin:0 auto 8px}.receipt-name{font-size:16px;font-weight:800;letter-spacing:1px;text-transform:uppercase}.receipt-tagline{font-size:10px;color:#555;margin-top:2px}.receipt-divider{text-align:center;color:#aaa;font-size:10px;margin:8px 0;letter-spacing:1px}.receipt-meta{margin:6px 0}.receipt-meta-row{display:flex;justify-content:space-between;margin-bottom:3px;font-size:11px}.receipt-items{width:100%;border-collapse:collapse;margin:6px 0;font-size:11px}.receipt-items th{font-size:10px;font-weight:700;text-transform:uppercase;padding:3px 0;border-bottom:1px solid #ccc}.receipt-items td{padding:4px 0;vertical-align:top}.receipt-items tr+tr td{border-top:1px dotted #e5e5e5}.receipt-totals{margin:6px 0}.receipt-total-row{display:flex;justify-content:space-between;font-size:11px;padding:2px 0}.receipt-total-row.grand{font-size:14px;font-weight:800;padding:6px 0 2px;border-top:2px solid #111;margin-top:4px}.receipt-payment{text-align:center;font-size:11px;margin:6px 0}.receipt-notes{font-size:10px;color:#555;font-style:italic;margin:4px 0;text-align:center}.receipt-footer{text-align:center;font-size:10px;color:#666;margin-top:14px;border-top:1px dashed #ccc;padding-top:10px}@media print{body{margin:0}.receipt{width:auto;padding:4px}.page-break{page-break-after:always;border:none;margin:0;padding:0}button{display:none!important}}</style></head><body><div style="text-align:center;padding:10px 0 6px;font-size:11px;color:#666;border-bottom:2px solid #eee;margin-bottom:16px">Printed ${new Date().toLocaleString()} · ${orders.length} receipt${orders.length>1?'s':''}<br><button onclick="window.print()" style="margin-top:8px;padding:6px 20px;font-size:12px;cursor:pointer;background:#111;color:#fff;border:none;border-radius:4px">🖨️ Print</button></div>${receiptsHtml}</body></html>`);win.document.close();setTimeout(()=>win.focus(),200);}
